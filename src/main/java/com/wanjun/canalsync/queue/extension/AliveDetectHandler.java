 ...
 * <p>
 * 检查正在执行的任务是否还在执行（存活），
 * 为了防止耗时比较久的任务（任务的执行时间超出了通过队列管理器配置的任务执行超时时间 - 默认值：{@link com.wanjun.canalsync.queue.config.Constant#PROTECTED_TIMEOUT}）
 * 会被备份队列监听器检测到并重新放入任务队列执行（因为备份队列监听器会把超出通过队列管理器配置的任务执行超时时间的任务当作是失败的任务（参考 https://github.com/fnpac/KMQueue#什么是失败任务？）并进行重试）。
 * <p>
 * 通过这种检测机制，可以保证{@link(Task)}返回为true的任务（任务还在执行）不会被备份队列监听器重新放入任务队列重试。
 * <p>
...
public interface AliveDetectHandler {
    ...
    String ALIVE_KEY_SUFFIX = "_alive";
